#!/usr/bin/env python3
"""nadine2016-arctl-init-stdio MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="nadine2016-arctl-init-stdio MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="http",
    ...
    try:
        ...
        server = DynamicMCPServer(
            name="nadine2016-arctl-init-stdio",
            tools_dir="src/tools"
        )
