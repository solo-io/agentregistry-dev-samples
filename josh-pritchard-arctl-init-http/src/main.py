#!/usr/bin/env python3
"""josh-pritchard-arctl-init-http MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="josh-pritchard-arctl-init-http MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="http",
    ...
    try:
        ...
        server = DynamicMCPServer(
            name="josh-pritchard-arctl-init-http",
            tools_dir="src/tools"
        )
