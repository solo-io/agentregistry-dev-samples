#!/usr/bin/env python3
"""danielle-mustillo-arctl-init-http MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="danielle-mustillo-arctl-init-http MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="http",
    ...
    try:
        ...
        server = DynamicMCPServer(
            name="danielle-mustillo-arctl-init-http",
            tools_dir="src/tools"
        )
