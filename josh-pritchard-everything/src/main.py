#!/usr/bin/env python3
"""josh-pritchard-everything MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="josh-pritchard-everything MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="stdio",
    ...
    try:
        ...
        server = DynamicMCPServer(
            name="josh-pritchard-everything",
            tools_dir="src/tools"
        )
