# josh-pritchard-everything

josh-pritchard-everything is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i josh-pritchard-everything:latest
   ```
