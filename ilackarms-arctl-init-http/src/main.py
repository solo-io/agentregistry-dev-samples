#!/usr/bin/env python3
"""ilackarms-arctl-init-http MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="ilackarms-arctl-init-http MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="http",
    ...
    try:
        ...
        server = DynamicMCPServer(
            name="ilackarms-arctl-init-http",
            tools_dir="src/tools"
        )
