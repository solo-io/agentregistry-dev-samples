# lcagw-arctl-init-stdio

lcagw-arctl-init-stdio is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container** (Dockerfile sets `HOST=0.0.0.0` so the http default is reachable):
   ```bash
   # HTTP (default)
   docker run -p 3000:3000 lcagw-arctl-init-stdio:latest

   # Stdio (pipe to an MCP client)
   docker run -i lcagw-arctl-init-stdio:latest --transport stdio
   ```
